...
\raisebox{-1pt}{\includegraphics[height=9pt]{orcid_icon.pdf}}}}
...
The previous oncology trial infrastructure suffered from several structural limitations that hindered both efficiency and patient safety:
...
This paper presents the National MCP Physical AI Oncology Trials system (repository version 1.1.0), a proposed end-to-end architecture that addresses the limitations of prior trial infrastructure through five key contributions:
...
The repository and this paper were constructed using an AI-assisted development methodology. Kevin Kawchak served as the prompt creator and system architect, directing AI code generation through structured prompts documented in \texttt{prompts.md}.
...
\small
\begin{tabularx}{\textwidth}{@{}L{3.2cm}L{2.5cm}R{1.2cm}L{5.8cm}@{}}
\toprule
...
\texttt{national-mcp-pai-oncology-trials}~\cite{kawchak2026national} & Mar 5--9, 2026 & 5 & National standard, SDKs, conformance suite, governance \\
\bottomrule
\end{tabularx}
\end{table}
...
\item \textbf{v1.1.0 (March 9, 2026):} This paper, updated documentation, and release notes.
\end{itemize}

\subsubsection{Development Metrics}

Table~\ref{tab:dev-metrics} presents quantitative development metrics derived from release dates and version identifiers:
...
Number of AI code generation sessions & 6 versions \\
...
Peer review was conducted using OpenAI ChatGPT 5.4 Thinking, which analyzed the repository structure and provided recommendations documented in the \texttt{peer-review/} directory. Five review rounds by Claude (\texttt{claude-1-2026-03-06.md} through \texttt{claude-5-2026-03-06.md}) and two by OpenAI (\texttt{openai-1-2026-03-06.md}, \texttt{openai-2-2026-03-06.md}) addressed gaps in:
...
\small
\begin{tabularx}{\textwidth}{@{}L{2.4cm}C{0.8cm}C{1.2cm}L{7.5cm}@{}}
\toprule
...
\small
\begin{tabularx}{\textwidth}{@{}L{3.5cm}L{3.8cm}L{4.5cm}@{}}
\toprule
...
Table~\ref{tab:repo-scale} summarizes the quantitative scope of the national repository at version 1.1.0:
...
\begin{enumerate}[leftmargin=*]
\item \textbf{Individual Docker containers:} Five Dockerfiles in \texttt{deploy/docker/} (e.g., \texttt{Dockerfile.authz}, \texttt{Dockerfile.fhir}).
...
\label{tab:schemas}
\small
\begin{tabularx}{\textwidth}{@{}L{4cm}L{2cm}L{5.5cm}@{}}
\toprule
...
\begin{verbatim}
  ...
  +---+----+    +--------------+    +-----------+
      ...
      v
  +--------+
  | ROBOT  |  Physical AI performs clinical task
...
\begin{itemize}[leftmargin=*]
\item \texttt{national-mcp-pai-oncology-trials} (v1.1.0)~\cite{kawchak2026national}
